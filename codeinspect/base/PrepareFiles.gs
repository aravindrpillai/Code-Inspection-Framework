...
uses java.io.File
uses codeinspect.util.RulesXMLUtil
...
class PrepareFiles {
  ...
  static final var _logger = LoggerUtil.getLogger(PrepareFiles)
  ...
  private function prepareEntityList(fileTypeDTO : FileTypeDTO) {
    var entityRootFolder = FileUtil.ConfigurationFolder + Constants.EntityRelativePath
  ...
  private function prepareTypelistList(fileTypeDTO : FileTypeDTO) {
    var entityRootFolder = FileUtil.ConfigurationFolder + Constants.TypelistRelativePath
    if (fileTypeDTO.ReadAllFilesOfThisType) {
      var entitiesPackage = new File(entityRootFolder)
      for (file in entitiesPackage.listFiles()) {
        if (file.File) {
  ...
  private function prepareRulesList(fileTypeDTO : FileTypeDTO) {
    var rulesRootFolder = FileUtil.ConfigurationFolder + Constants.RulesRelativePath
    if (fileTypeDTO.ReadAllFilesOfThisType) {
  ...
  private function preparePCFList(fileTypeDTO : FileTypeDTO) {
    var entityRootFolder = FileUtil.ConfigurationFolder + Constants.PCFRelativePath
    if (fileTypeDTO.ReadAllFilesOfThisType) {
