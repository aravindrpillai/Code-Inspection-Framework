...
abstract class CodeInspectBase extends CodeInspectCore {
  ...
  private var _logger = LoggerUtil.getLogger(CodeInspectBase)
  ...
  final function run(fileMetaData : FileMetadataDTO, rule : RuleDTO) {
    ...
    if (shouldExecute(fileMetaData)) {
      _lock.lock()
      try {
        var lines = java.nio.file.Files.readAllLines(java.nio.file.Paths.get(fileMetaData.File.AbsolutePath))
