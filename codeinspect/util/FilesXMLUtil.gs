...
uses codeinspect.dto.LocationDTO
...
class FilesXMLUtil {
  ...
  static final var _logger = LoggerUtil.getLogger(FilesXMLUtil)
  ...
  static function getFileDetails(fileXMLLocation : String) : List<FileTypeDTO> {
    if (fileXMLLocation == null) {
      ...
      fileXMLLocation = FileUtil.AppFolder.concat("/codeinspect/constants/files.xml")
    }
    ...
    foreach (contentIndex in 0..|files.Length) {
      ...
      if(fileType == DISPLAYKEY){
        ...
        fileDTO.Locations = { new LocationDTO(){ :Location = Constants.DisplayKeyRelativePath.concat("/display.properties"), :IsFolder = false }}
